...
export async function routes(app: FastifyInstance) {
    ...
    app.post('/notes', { preHandler: auth }, async (req, res) => {
        ...
        const submitted_note = req.body as { title_cypher: string; note_cypher: string; key_hash: string };
    ...
    app.get('/notes/:id', { preHandler: auth }, async (req, res) => {
        ...
        return user === decrypt(note.sec_hash!, note_key)
            ? res.status(200).send({ note: note.content })
            : res.status(400).send({ error: 'Invalid Auth' });
    });
}